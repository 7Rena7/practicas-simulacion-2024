...
if len(arguments) != 9 or arguments[1] != "-c" or arguments[3] != "-n" or arguments[5] != "-e" or arguments[7] != "-s" or arguments[8] != "-a":
...
# Llamar a cada función según la estrategia seleccionada
if estrategia == 'm':
    # Initialize a new instance of Martingale class and execute the strategy
...
def graficar_frecuencia(frecuencia_relativa_listado, es_promedio):
    ...
    for frecuencia_relativa in frecuencia_relativa_listado:
        ax.plot([i for i in range(cantidad_tiradas)],
                frecuencia_relativa, linewidth=2.0, )

    ax.axhline(y=1 / 37, color='r', linestyle='--',
               label='frn (frecuencia relativa esperada)')

    ax.legend()

    if es_promedio:
        ax.set_title(
            f"FRECUENCIA RELATIVA DEL NUMERO {numero_elegido} PROMEDIO {cantidad_corridas} CORRIDAS")
    else:
        ax.set_title(
            f"FRECUENCIA RELATIVA DEL NUMERO {numero_elegido} {cantidad_corridas} CORRIDAS")
...
def graficar_varianza(varianza_listado, es_promedio):
    ...
    for varianza in varianza_listado:
        ax.plot([i for i in range(cantidad_tiradas)],
                varianza, linewidth=2.0, )

    ax.axhline(y=varianza_esperada, color='r', linestyle='--',
               label='vve (valor de la varianza esperada)')
...
def graficar_desvio(desviacion_estandar_listado, es_promedio):
    ...
    for desviacion_estandar in desviacion_estandar_listado:
        ax.plot([i for i in range(cantidad_tiradas)],
                desviacion_estandar, linewidth=2.0, )

    ax.axhline(y=desviacion_estandar_esperada, color='r',
               linestyle='--', label='vde (valor del desvio esperado)')
...
def graficar_esperanza(esperanza_matematica_listado, es_promedio):
    ...
    for esperanza_matematica in esperanza_matematica_listado:
        ax.plot([i for i in range(cantidad_tiradas)],
                esperanza_matematica, linewidth=2.0, )

    ax.axhline(y=esperanza_matematica_esperada, color='r',
               linestyle='--', label='vpe (valor promedio esperado)')

    ax.legend()

    if es_promedio:
        ax.set_title(
            f"ESPERANZA MATEMATICA PROMEDIO {cantidad_corridas} CORRIDAS")
    else:
...
for corrida in range(cantidad_corridas):
    ...
    frecuencia_relativa, esperanza_matematica, desviacion_estandar, varianza = generar_estadisticas(
        numero_elegido)
...
df_histograma_promedio["fr"] = df_histograma_promedio["fa"] / \
    (cantidad_tiradas * cantidad_corridas)
